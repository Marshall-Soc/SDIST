...
{synopt:{opt par2(#)}}The second parameter to be specified depending on the distribution selected in {cmd:type()}. Since the default {cmd:type()} is the rectangular distribution, the default is the  higher end of the [a,b) interval. The samples are generated through the {cmd:runiform()} function so the default for  is 1, but this can be changed. If {cmd:type(normal)} is selected, this parameter is the standard deviation, with a default of 1. If {cmd:type(poisson)} is selected, this parameter is the mean, also with a default of 0.{p_end}
{synopt:{opt round(#)}}the decimal point to which the estimates should be rounded. The default is 0.001.{p_end}
{synopt:{opt histplot}}Indicates whether or not histograms of the two frequency distributions should be plotted. The plots are generated through the {cmd:histogram} command (StataCorp n.d.3). Defaults to no histogram.{p_end}
...
{synopt:{opt combine}}Indicates whether or not the two histograms should be stacked to form a third plot. This is a call to the {cmd:graph combine} function (StataCorp n.d.5). Both histograms have to be saved in order for the graphs to be combined, either by specifying both {cmd:saveplot1()} and {cmd:saveplot2()} simultaneously or by specifying {cmd:repplot} without either saveplot options (though {cmd:repplot} can still be used in conjunction with both {cmd:saveplot1()} and {cmd:saveplot2()} if both are specified). Requiring that both {cmd:saveplot1()} and {cmd:saveplot2()} or {cmd:repplot} only be specified prevents the program from erroneously stacking histograms from different simulations. The default is to not stack the plots.{p_end}
{synopt:{opt lcolor}}Indicates the outline color of the histogram bars. This is a call to the {cmd:histogram} function (StataCorp n.d.3). The default is black.{p_end}
{synopt:{opt fcolor}}Indicates the interior color of the histogram bars. This is a call to the {cmd:histogram} function (StataCorp n.d.3). The default is gs6.{p_end}
{synopt:{opt bckg}}Indicates the color of the graph region background. This is a call to the {cmd:graphregion(fcolor())} argument within the {cmd:histogram} command (StataCorp n.d.4). The default is white.{p_end}
{synopt:{opt nlcolor}}Indicates the color of the normal curve line. This is a call to the {cmd:normopts(lcolor())} argument within the {cmd:histogram} command (StataCorp n.d.3). The default is black.{p_end}
{synopt:{opt nlwidth}}Indicates the thickness of the normal curve line. This is a call to the {cmd:normopts(lwidth())} argument within the {cmd:histogram} command (StataCorp n.d.3). The default is .5.{p_end}
{synopt:{opt nlpattern}}Indicates the pattern of the normal curve line. This is a call to the {cmd:normopts(lpattern())} argument within the {cmd:histogram} command (StataCorp n.d.3). The default is solid.{p_end}
...
{phang}StataCorp. N.d.2. Functions. College Station, TX: StataCorp LP. Accessed February 9, 2017 (http://www.stata.com/manuals13/dfunctions.pdf).{p_end}
{phang}StataCorp. N.d.3. histogram -- Histograms for continuous and categorical variables. College Station, TX: StataCorp LP. Accessed September 26, 2017 (https://www.stata.com/manuals13/rhistogram.pdf).{p_end}
{phang}StataCorp. N.d.4. region_options -- Options for shading and outlining regions are controlling graph size. College Station, TX: StataCorp LP. Accessed September 26, 2017 (https://www.stata.com/manuals13/g-3region_options.pdf).{p_end}
{phang}StataCorp. N.d.5. graph combine -- Combine multiple graphs. College Station, TX: StataCorp LP. Accessed September 26, 2017 (https://www.stata.com/manuals13/g-2graphcombine.pdf).{p_end}
